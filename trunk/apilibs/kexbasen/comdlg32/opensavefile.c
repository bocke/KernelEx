...
static UINT_PTR CALLBACK OFNHookProcWAdapter( LPOFNHOOKPROC pfnhook, LPOPENFILENAMEW lpofnW, HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uiMsg) {
	case WM_INITDIALOG:
	...
	case WM_NOTIFY:
		{
			...
			switch (notif->hdr.code)
			{
				...
				case CDN_TYPECHANGE:
				{
					if (notif->hdr.code == CDN_FILEOK || notif->hdr.code == CDN_SHAREVIOLATION) //pszFile is updated
					{
						if (notif->lpOFN->Flags & (OFN_EXPLORER|OFN_ALLOWMULTISELECT)) //many files could be selected, convert \0-separated list
							MultiByteToWideChar(CP_ACP,0,notif->lpOFN->lpstrFile,strlenAA(notif->lpOFN->lpstrFile),lpofnW->lpstrFile,lpofnW->nMaxFile);
						else
							MultiByteToWideChar(CP_ACP,0,notif->lpOFN->lpstrFile,-1,lpofnW->lpstrFile,lpofnW->nMaxFile);		
	...
	default:
		if ( uiMsg == WM_HELPMSGSTRING || uiMsg == WM_FILEOKSTRING || WM_SHAREVISTRING )
		{
			...
			if (uiMsg == WM_FILEOKSTRING || uiMsg == WM_SHAREVISTRING) //pszFile is updated
			{				
				if (lpofnW->Flags & (OFN_EXPLORER|OFN_ALLOWMULTISELECT)) //many files could be selected, convert \0-separated list
					MultiByteToWideChar(CP_ACP,0,lpofnA->lpstrFile,strlenAA(lpofnA->lpstrFile),lpofnW->lpstrFile,lpofnW->nMaxFile);
				else
					MultiByteToWideChar(CP_ACP,0,lpofnA->lpstrFile,-1,lpofnW->lpstrFile,lpofnW->nMaxFile);
			}
...
static void CALLBACK WinCreateEvent( HWINEVENTHOOK hWinEventHook, DWORD event, HWND hwnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime )
{
	//catch "Open"(Save) button creation, since on dialog window creation no dlgproc set yet
	if ( GetDlgCtrlID(hwnd) == IDOK )
	{
		hwnd = GetParent(hwnd);
		//mark dialog created as unicode. really, everything should be marked unicode with thunilay
		//but then dialog may choke on unicode common controls
		if ( GetClassWord(hwnd,GCW_ATOM) == DIALOG_ATOM && GetProp(hwnd,OLDDLGPROCPROP) == NULL )
		{
...
static BOOL WINAPI GetOpenOrSaveFileNameW(LPOPENFILENAMEW lpofn, BOOL IsSave)
{
	...
	if (lpofn->lpstrFile)
	{
		...
		WideCharToMultiByte(CP_ACP,0,lpofn->lpstrFile,-1,ofnA.lpstrFile,ofnA.nMaxFile,NULL,NULL);
	}
	...
	HWINEVENTHOOK eventhook = SetWinEventHook(EVENT_OBJECT_CREATE, EVENT_OBJECT_CREATE, GetModuleHandle("USER32.DLL"),
		(WINEVENTPROC)WinCreateEvent, GetCurrentProcessId(), GetCurrentThreadId(), WINEVENT_INCONTEXT);
